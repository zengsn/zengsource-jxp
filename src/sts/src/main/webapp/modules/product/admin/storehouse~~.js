...
function CreateProductEditForm(config) {
	return new Ext.form.FormPanel({
		id: config.id,
		iconCls: 'icon-be-product',
	    title: config.title,
	    bodyStyle:'padding:15px',
	    closable: !config.closable ? false : config.closable,
	    //width: 350,
	    collapsible: false,
	    region:'center',
	    margins: '5 0 5 5',
...
var editProduct = function(id) {
	...
	var productEditForm = CreateProductEditForm({
		id: 'product-form-' + idHash,
		title: title,
		closable: true
	});
	var productTabPanel = Ext.getCmp('product-storehouse-panel');
	if(!productTabPanel.findById(productEditForm.id)) {
		productTabPanel.add(productEditForm);
	}
	productTabPanel.activate(productEditForm.id);
};
// create the grid
...
// Product Add Form
var productAddForm = CreateProductEditForm({
	id: 'product-form-add',
	title: '创建新产品'
}); 
...
var productLayout = new Ext.Panel({
