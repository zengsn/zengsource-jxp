/*
 * Rewrite SearchField of ExtJS document.
 * Copyright(c) 2008, snzeng
 * z.sn@live.cn
 */
Ext.ux.SearchField = function(config) {
	...
	config = Ext.applyIf(config || {}, {
		restore: function() {
			config.store.baseParams[config.paramName] = '';
			config.store.removeAll();
			//config.focus();
		}
    });

	Ext.ux.SearchField.superclass.constructor.apply(this, arguments);
};
