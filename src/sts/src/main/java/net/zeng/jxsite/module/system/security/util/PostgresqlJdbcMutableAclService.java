 ...
 */
public class PostgresqlJdbcMutableAclService extends PostgresqlJdbcAclService implements
		MutableAclService {
	// ~ Instance fields
	// ========================================================
	// ========================================
	...
	public PostgresqlJdbcMutableAclService(DataSource dataSource, LookupStrategy lookupStrategy,
			AclCache aclCache) {
	...
	/**
	 * Creates a new row in acl_entry for every ACE defined in the passed
	 * MutableAcl object.
	 * 
	 * @param acl
	...
	/**
	 * Creates an entry in the acl_object_identity table for the passed
	 * ObjectIdentity. The Sid is also necessary, as acl_object_identity has
	 * defined the sid column as non-null.
	 * 
	 * @param object
	 *            to represent an acl_object_identity for
	 * @param owner
	 *            for the SID column (will be created if there is no acl_sid
	 *            entry for this particular Sid already)
	 */
	protected void createObjectIdentity(ObjectIdentity object, Sid owner) {
		...
		jdbcTemplate.update(insertObjectIdentity, new Object[] { classId, objectId, sidId,
				new Boolean(true) });
	}

	/**
	 * Retrieves the primary key from acl_class, creating a new row if needed
	 * and the allowCreate property is true.
	 * 
	 * @param clazz
	 *            to find or create an entry for (this implementation uses the
	...
	protected Long createOrRetrieveClassPrimaryKey(Class<?> clazz, boolean allowCreate) {
		List<?> classIds = jdbcTemplate.queryForList(selectClassPrimaryKey, new Object[] { clazz
				.getName() }, Long.class);
		Long classId = null;
	...
	/**
	 * Retrieves the primary key from acl_sid, creating a new row if needed and
	...
	protected Long createOrRetrieveSidPrimaryKey(Sid sid, boolean allowCreate) {
		...
		List<?> sidIds = jdbcTemplate.queryForList(selectSidPrimaryKey, new Object[] {
				new Boolean(principal), sidName }, Long.class);
		Long sidId = null;
	...
	public void deleteAcl(ObjectIdentity objectIdentity, boolean deleteChildren)
		...
		if (deleteChildren) {
			...
			throw new ChildrenExistException("Cannot delete '" + objectIdentity + "' (has "
	...
	/**
	 * Deletes all ACEs defined in the acl_entry table belonging to the
	 * presented ObjectIdentity
	 * 
	 * @param oid
	 ...
	 * presented ObjectIdentity. In addition, deletes the corresponding row from
	 * acl_class if there are no more entries in acl_object_identity that use
	 * that particular acl_class. This keeps the acl_class table reasonably
	 * small.
	 * 
	 * @param oid
	 *            to delete the acl_object_identity (and clean up acl_class for
	...
	protected void deleteObjectIdentityAndOptionallyClass(ObjectIdentity oid) {
		...
		jdbcTemplate.update(deleteObjectIdentityByPrimaryKey,
				new Object[] { retrieveObjectIdentityPrimaryKey(oid) });

		// Delete the acl_class row, assuming there are no other references to
		// it in acl_object_identity
		Object[] className = { oid.getJavaType().getName() };
		long numObjectIdentities = jdbcTemplate.queryForLong(
				selectCountObjectIdentityRowsForParticularClassNameString, className);
	...
	/**
	 * Retrieves the primary key from the acl_object_identity table for the
	 * passed ObjectIdentity. Unlike some other methods in this implementation,
	 * this method will NOT create a row (use
	...
	/**
	 * This implementation will simply delete all ACEs in the database and
	 * recreate them on each invocation of this method. A more comprehensive
	 * implementation might use dirty state checking, or more likely use ORM
	...
	/**
	 * Updates an existing acl_object_identity row, with new information
	 * presented in the passed MutableAcl object. Also will create an acl_sid
	 * entry if needed for the Sid that owns the MutableAcl.
	 * 
	 * @param acl
